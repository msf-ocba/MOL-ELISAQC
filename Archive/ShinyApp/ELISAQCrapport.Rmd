...
### Function to extract the end of a string by n characters:
...
elisa_dtstamp <- fread(file = params$resultats, 
                         stringsAsFactors = FALSE, 
                         skip = 1, 
                         select = 1, 
                         nrows = 1, 
...
### convert time stamp to date time string:
elisa_dtstamp <- as.POSIXlt(elisa_dtstamp[, V1], format = "%d/%m/%Y %H:%M")


### Import MOL reference data:
...
# Add Laboratory name:
elisaout[, Laboratoire := "MOL"]

# Add sample ID:
elisaout[, SID := paste0("S", endstring(PID, 3), "_01")]

# Add sample type:
elisaout[, Échantillon_type := "sérum"]

# Add date time stamps:
elisaout[, Date_prélèvement := format(elisa_dtstamp - lubridate::days(1), "%Y-%m-%d")]
elisaout[, Date_résultat := format(elisa_dtstamp, "%Y-%m-%d")]

# Add Platform:
elisaout[, Plateforme := "ELISA"]

# Add kit details:
kitdet <- unlist(strsplit(params$kit, split = " \\("))
...
# Add test status:
elisaout[, Statut_test := data.table::fifelse(qc == "reussi", "Terminé", "En cours")]

# Add updated at field to overwrite:
elisaout[, Updated_at := Sys.time()]

# Update column order:
setcolorder(elisaout, c("Laboratoire", 
                        "Operateur",
                        "PID",
                        "SID",
                        "Date_prélèvement",
...
# Export the sub-setted data (must be .xlsx with dates in YYYY-MM-DD format for Go.Data):
write.xlsx(x = elisaout, 
           file = here("Output", paste0("Résultats_", params$kit, "_", today, ".xlsx")),
           asTable = FALSE, 
           overwrite = TRUE)
